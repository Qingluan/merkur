...
func (c *Conn) Request() error {
	...
	// padding
	if paddingLen > 0 {
		padding := tcp.GetBuffer(paddingLen)
		rand.Read(padding)
		buf.Write(padding)
		tcp.PutBuffer(padding)
	}
